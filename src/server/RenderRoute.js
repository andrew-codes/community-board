...
function renderRoute(routerState, transition) {
    const redux = createStore(stores, {}, ({getState, dispatch}) => [loggerMiddleware, thunkMiddleware(getState), promiseMiddleware]);
	...
	return Promise.all(RouteToInitialData(redux, routerState))
		.then(()=> {
			var appHtml = React.renderToString(<Root store={redux} routerState={routerState} />);
			var scriptsHtml = `
